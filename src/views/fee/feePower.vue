    ...
    <isTable :list="feeList" :listInfo="listInfo" :height="height">
      <template v-slot:btn="scope">
        ...
        <el-button
          ...
          icon="el-icon-delete"
          size="mini"
          v-show="scope.row.payPowerStatus == 0"
          @click="pay(scope.row)"
          >缴费</el-button
        >
      </template>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        powerNum: [{ required: true, message: "请填写表显", trigger: "blur" }],
        payPowerStatus: [
